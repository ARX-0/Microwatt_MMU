...
```
a2o/rel/src/verilog/work/mmq_rtw.v     the radix walker
a2o/rel/src/verilog/work/mmq*.v        the rest of the A2O MMU
a2o/rel/src/verilog/sim/               testbenches + run_rtw_tests.sh
a2o/golden/src/                        pristine upstream snapshot, for diffing
microwatt/mmu.vhdl                     the reference implementation
tools/a2o-diff.sh                      what changed vs upstream
```

To reproduce the test results:

```bash
a2o/rel/src/verilog/sim/run_rtw_tests.sh   # lint + both benches, ~5 seconds
tools/a2o-diff.sh --stat                   # every line changed vs upstream A2O
```
