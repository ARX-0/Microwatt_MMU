...
No simulation infrastructure existed in this repository — `a2o/rel/build/` contains only
...
```bash
a2o/rel/src/verilog/sim/run_rtw_tests.sh
```
...
```bash
cd a2o/golden/src/verilog && verilator --lint-only --language 1364-2005 \
    -Wno-fatal -Wno-LITENDIAN -Wno-DECLFILENAME -Wno-TIMESCALEMOD \
    -Itrilib -Iwork work/mmq.v --top-module mmq 2>&1 | grep -c '^%Error'   # 3
cd a2o/rel/src/verilog    && verilator ... same ...                        # 3
```

## 6.7 The golden-tree workflow

`a2o/golden/src/` is a byte-identical snapshot of the upstream A2O source, committed
alongside the editable tree. `tools/a2o-diff.sh` answers *"what has changed since
upstream"* — a different question from `git diff`, and the one that matters when a port
touches a handful of places inside several 4000-line files:

```bash
tools/a2o-diff.sh                  # summary of changed files
tools/a2o-diff.sh --stat           # per-file line counts
tools/a2o-diff.sh mmq_tlb_cmp.v    # unified diff of one file
tools/a2o-diff.sh --patch          # whole-tree unified diff
```

It also keeps every `file:line` citation in this documentation stable: the golden tree does
not move as editing continues.
